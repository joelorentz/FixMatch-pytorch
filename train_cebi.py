...
from pytorch_utils import initialize_model, LrFinder
from pytorch_utils.plot import show_examples_multiview
...
# ONLY necessary for multi-GPU, make sure each gpu receives bacthes with identical split of L and U samples
...
def main():
    ...
    if args.find_lr:
        lr_finder = LrFinder(model, optimizer, labeled_trainloader,nn.CrossEntropyLoss())
    ...
    # logger.info(f"  Task = Cebi@{args.num_labeled}")
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
          model, optimizer,scheduler, ema_model=None):
    if args.amp:
        from apex import amp
    global best_acc
    test_accs = []
    ...
    # show_examples_multiview(labeled_trainloader,8,CEBI_MEAN,CEBI_STD)
    # show_examples_multiview(test_loader,8,CEBI_MEAN,CEBI_STD)

    # show_unalbeled_examples(unlabeled_trainloader,4,cebi_mean,cebi_std)

    x_crit = CrossEntropyLoss()
    u_crit = CrossEntropyLoss(reduction="none")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.set_grad_enabled(True):
            for batch_idx in range(args.eval_step):
                ...
                # inputs_x, targets_x = labeled_trainloader.next()
                except:
                    labeled_iter = iter(labeled_trainloader)
                    inputs_x, targets_x, _ = labeled_iter.next()
                ...
                # # batch := bs labeled, 2*mu*bs unlabeled (weak and strong)
                # inputs = interleave(
                ...
                Lx = x_crit(logits_x, targets_x)

                # pseudo_label = torch.softmax(logits_u_w.detach()/args.T, dim=-1) # temp scaling
                # max_probs, targets_u = torch.max(pseudo_label, dim=-1)
                # mask = max_probs.ge(args.threshold).float() # only above threshold

                # # # Lu = (F.cross_entropy(logits_u_s, targets_u,
                # # #                       reduction='none') * mask).mean()
                # Lu = (u_crit(logits_u_s, targets_u) * mask).mean()
                ...
                # unlabeled
                inputs = torch.cat((inputs_u_s,inputs_u_w))
                inputs = inputs.to(args.device)
                outputs = model(inputs)
                logits_u_w, logits_u_s = outputs.chunk(2)
                pseudo_label = torch.softmax(logits_u_w.detach()/args.T, dim=-1) # temp scaling
                max_probs, targets_u = torch.max(pseudo_label, dim=-1)
                mask = max_probs.ge(args.threshold).float() # only above threshold
